...
{{-- <br>
        ...
        <div class="sorts">
            <div class="container">
                @if( $products->count() == 0 ) <h1 class="text-center mb-5">    {{__('This section does not currently contain items.')}} </h1>@endif
                        <h1 class="text-center mb-5">@if($category->name_en != null)
                                        @if( LaravelLocalization::getCurrentLocaleDirection() == 'rtl')
                                        {{$category->name}}
                                        @else
                                        {{$category->name_en}}
                            ...
                            @foreach($products as $product)
                                <div class="sort text-center relative">
                                    ...
                                    <div class="d-flex justify-content-between mt-3 px-5 buttons-dev">
                                        <h5 class="price-regular">{{$product->price}} {{__('AED')}}</h5>
                                        <a class="btn btn-cart add_cart" product_id="{{$product->id}}" href="#">{{__('Add to cart')}}</a>
                                        <a  product_id="{{$product->id}}" href="{{route('viewProperty',$product->id)}}">{{ __('Read more') }}</a>
