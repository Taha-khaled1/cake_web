                                            ...
                                            <div class="dropdown">
                                                <button class="btn btn-secondary dropdown-toggle" type="button" id="statusDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                                                    ...
                                                    @elseif( $c->payment_method == 'failed')
                                                        <span class="badge bg-success">تم الغاء الدفع</span>
                                                    @else
                                                        <span class="badge bg-danger">الدفع عند الاستلام</span>
                                                    @endif
                                                </button>
                                                <ul class="dropdown-menu" aria-labelledby="statusDropdown">
                                                    <li><form method="POST" action="{{route('update.orderpay', [$c->id , 'pending'])}}">@csrf <button type="submit" class="dropdown-item">في انتظار الدفع</button></form></li>
                                                    <li><form method="POST" action="{{route('update.orderpay', [$c->id , 'completed'])}}">@csrf <button type="submit" class="dropdown-item">تم الدفع</button></form></li>
                                                    <li><form method="POST" action="{{route('update.orderpay', [$c->id , 'failed'])}}">@csrf <button type="submit" class="dropdown-item">تم الغاء الدفع</button></form></li>
                                                    <li><form method="POST" action="{{route('update.orderpay', [$c->id , 'cash'])}}">@csrf <button type="submit" class="dropdown-item">الدفع عند الاستلام</button></form></li>
                                                </ul>
                                            </div>
                                        </td>
