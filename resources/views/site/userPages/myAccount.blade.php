                                                ...
                                                <div class="myaccount-content">
                                                    ...
                                                    <div class="myaccount-table table-responsive text-center">
                                                        <table class="table table-bordered">
                                                            ...
                                                            <tbody>
                                                                @foreach(\Illuminate\Support\Facades\Auth::user()->Special_order as $order)
                                                              <tr class="R_sorder{{$order->id}}">
                                                              <th scope="row">{{$loop->iteration}}</th>
                                                              <td>  {{$order->flavor}} </td>
                                                              <td>   {{$order->Weight}} </td>
                                                                
                                                                <th > @if($order->status == 0) {{__('Rejected')}}@endif @if($order->status == 1) {{__('Requested')}} @endif @if($order->status == 2)  {{__('is shipped')}} @endif @if($order->status == 3) {{__('Delivered')}} @endif </th>
                                                                <th>   @if($order->status == 1)<a href="" class="deletem_sorder" deletem_order="{{$order->id}}" > {{__('cancel')}}</a>@endif </th>
                                                                <td>    @if($order->img)
                                                                <img src="{{asset('/storage/property/'.$order->img)}}"   alt="{{$order->id}}" width="100">
                                                                @endif  </td>
                                                                                        <th >   @if($order->design)
                                                                <img src="{{asset('/storage/property/'.$order->design)}}"   alt="{{$order->id}}" width="100">
                                                                @endif  </th>
                                                              </tr>
                                                              @endforeach
                                                            </tbody>
